...
">",Math.min(b[0],b[1]),c);this.where(a,"<",Math.max(b[0],b[1]),c)},c);return this},where_like:function(a,b,c){this.where(a,"like",b,c);return this},limit:function(a,b){1==arguments.length?__bindings.limit=[0,Math.max(0,a)]:1<arguments.length&&(__bindings.limit=[Math.max(0,a),Math.max(0,a+b)]);return this},get:function(){var a=[],b=Algorithm.convert_columns(__data[0],__bindings.fields);0<__bindings.where.conditions.length&&(a=Algorithm.filter(__data,function(a){return __bindings.where.isMatch(a)?
a:!1}));a=0<__bindings.order_by.length?Algorithm.order_by(a,__bindings.order_by,0,function(a){return Algorithm.get_row_data(a,b)}):Algorithm.filter(a,function(a){return Algorithm.get_row_data(a,b)});0<__bindings.limit.length&&(a=a.slice(__bindings.limit[0],Math.min(__bindings.limit[1],a.length)));Log("List--",a);Log("SQL--",this.getSql());this.clear();return a},find:function(a){a=this.where(a).get();return 0<a.length?a[0]:!1},fetch:function(a){var b=this.get();return Helper.isFunction(a)?(Log("fetch begin"),
